...
{:else}
		...
		{#snippet stepDefaultsHint()}
			<div class="defaults-block">
				<p class="defaults-copy">
					Example values fill only empty or invalid fields on this step. The rest of your answers
					stay as they are.
				</p>
				<TextLinkButton
			...
			<div class="form nothing-surface">
				...
				<label class="field">
					...
					<select
						...
						onchange={(e) =>
							patchTraining({ fitness_level: (e.target as HTMLSelectElement).value })}
					>
						<option value="">Select</option>
			...
			<div class="form nothing-surface">
				...
				<label class="field">
					...
					<select
						...
						onchange={(e) =>
							patchLifestyle({ stress_level: (e.target as HTMLSelectElement).value })}
					>
						<option value="">Select</option>
	...
	{#if reviewOpen}
		<div class="modal" role="presentation">
			<button type="button" class="backdrop" aria-label="Close review" onclick={closeReview}
			></button>
			<div
			...
			>
				...
				<ul class="rev-ul">
					...
					<li>
						<strong>Training</strong>: {$onboarding.training.days_per_week}×/wk, {$onboarding
							.training.location || '—'}
					</li>
				</ul>
				<div class="rev-row">
